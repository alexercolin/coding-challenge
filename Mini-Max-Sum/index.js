...
function miniMaxSum(arr) {
  ...
  function firstSum(sortedArray, arr) {
    arr.pop();
    const sumArrayWithoutLastlement = arr.reduce(
      (firstSum, index) => firstSum + index
    );

    firstArray.shift();
    const sumArrayWithoutFirstElement = firstArray.reduce(
      (secondSum, index) => secondSum + index
    );
